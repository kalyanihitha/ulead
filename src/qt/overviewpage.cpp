...
// Copyright (c) 2017-2018 The ULead developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ULEAD)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sULEADPercentage, QString& szULEADPercentage)
{
    int nPrecision = 2;
    ...
    szULEADPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sULEADPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount uleadAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = uleadAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // ULEAD labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, uleadAvailableBalance, false, BitcoinUnits::separatorAlways));
   ...
   // ui->labelzBalanceImmature->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, immatureZerocoinBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelULEADPercent->setText(sPercentage);
   // ui->labelzULEADPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zULEAD.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in ulead.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in ulead.conf");
    }
    ...
    bool showULEADAvailable = settingShowAllBalances || uleadAvailableBalance != nTotalBalance;
    bool showWatchOnlyULEADAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showULEADPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyULEADPending = watchUnconfBalance != 0;
    bool showULEADLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyULEADLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showULEADAvailable || showWatchOnlyULEADAvailable);
    ui->labelBalanceText->setVisible(showULEADAvailable || showWatchOnlyULEADAvailable);
    ui->labelWatchAvailable->setVisible(showULEADAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showULEADPending || showWatchOnlyULEADPending);
    ui->labelPendingText->setVisible(showULEADPending || showWatchOnlyULEADPending);
    ui->labelWatchPending->setVisible(showULEADPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showULEADLocked || showWatchOnlyULEADLocked);
    ui->labelLockedBalanceText->setVisible(showULEADLocked || showWatchOnlyULEADLocked);
    ui->labelWatchLocked->setVisible(showULEADLocked && showWatchOnly);
    ...
    bool showzULEADAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzULEADUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzULEADImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
   // ui->labelzBalanceMature->setVisible(showzULEADAvailable);
   // ui->labelzBalanceMatureText->setVisible(showzULEADAvailable);
   // ui->labelzBalanceUnconfirmed->setVisible(showzULEADUnconfirmed);
   // ui->labelzBalanceUnconfirmedText->setVisible(showzULEADUnconfirmed);
   // ui->labelzBalanceImmature->setVisible(showzULEADImmature);
   // ui->labelzBalanceImmatureText->setVisible(showzULEADImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelULEADPercent->setVisible(showPercentages);
   // ui->labelzULEADPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ULEAD")
    updateDisplayUnit();
}
