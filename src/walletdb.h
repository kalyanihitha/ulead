...
public:
    ...
    bool WriteZULEADSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZULEADSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZULEADSeed_deprecated(uint256& seed);
    bool EraseZULEADSeed();
    bool EraseZULEADSeed_deprecated();

    bool WriteZULEADCount(const uint32_t& nCount);
    bool ReadZULEADCount(uint32_t& nCount);
